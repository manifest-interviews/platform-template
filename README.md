...
There is nothing to build or set up.

1. Fork the repo.
2. Get to know the service the way you'd size up any system you were about to
   operate. Everything you need is in the repo; running it locally
   (`docker compose up --build`) is optional — the fastest way to see the
   moving parts, but reading gets you there too.
...
infra/pulumi/    Pulumi project (empty — nothing is deployed)
...
- CI runs test → build → deploy; deploy publishes the image to GitHub Container
  Registry.
- `infra/pulumi/` is an empty Pulumi project — nothing is deployed today.
